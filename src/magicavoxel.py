...
def write_matl_chunk(material):

    chunk_content = struct.pack('i', len("_type".encode('utf-8')))
    chunk_content += struct.pack('i', len("_type".encode('utf-8')))
    chunk_content += struct.pack('i', len("_type".encode('utf-8')))
    chunk_content += bytearray("_type".encode('utf-8'))
    chunk_content += struct.pack('i', len("_metal".encode('utf-8')))
    chunk_content += bytearray("_metal".encode('utf-8'))

    chunk_content += struct.pack('i', len("_rough".encode('utf-8')))
    chunk_content += bytearray("_rough".encode('utf-8'))
    chunk_content += struct.pack('i', len("0.1".encode('utf-8')))
    chunk_content += bytearray("0.1".encode('utf-8'))

    chunk_content += struct.pack('i', len("_ior".encode('utf-8')))
    chunk_content += bytearray("_ior".encode('utf-8'))
    chunk_content += struct.pack('i', len("0.3".encode('utf-8')))
    chunk_content += bytearray("0.3".encode('utf-8'))
    
    return write_chunk(b'MATL', chunk_content, None)  

def write_main_chunk(volume_list, palette, materials):
    ...
    #     child_chunks += write_matl_chunk(material)
...
def write(volume_list, filename, palette = None, materials = None):
    ...
    data = data + write_main_chunk(volume_list, palette, materials)
